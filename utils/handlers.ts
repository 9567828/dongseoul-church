...
export const handlers = () => {
  ...
  const handleAuthReset = async (email: string) => {
    await request({ method: "POST", url: "/auth/reset-password", data: email });
  };

  return { handleCheckedRole, toggleAllChecked, handleAdminInvite, handleAuthReset };
};
